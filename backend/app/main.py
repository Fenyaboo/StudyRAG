...
async def root_ready(request: Request) -> JSONResponse:
    ...
    configured = (
        request.app.state.storage.configured
        and request.app.state.dify.configured
        and request.app.state.embedding.configured
    )
    return JSONResponse({"status": "ready" if database and configured else "not_ready"})
