-- Canonical migration is maintained at ../../../../supabase/migrations/001_init.sql.
-- Keep this copy available for backend-only deployments.
...
CREATE TABLE IF NOT EXISTS public.documents (
    ...
    subject TEXT NOT NULL DEFAULT 'Chung' CHECK (subject IN ('Toán', 'Lý', 'Hóa', 'Chung')),
    doc_type TEXT NOT NULL DEFAULT 'exam' CHECK (doc_type IN ('exam', 'textbook')),
    status TEXT NOT NULL DEFAULT 'processing' CHECK (status IN ('processing', 'ready', 'failed', 'ocr_required')),
...
CREATE INDEX IF NOT EXISTS idx_chunks_vec ON public.document_chunks USING hnsw (embedding extensions.vector_cosine_ops) WITH (m = 16, ef_construction = 64);
...
CREATE POLICY owner_documents ON public.documents FOR ALL USING (owner_id = auth.uid()) WITH CHECK (owner_id = auth.uid());
DROP POLICY IF EXISTS owner_chunks ON public.document_chunks;
CREATE POLICY owner_chunks ON public.document_chunks FOR ALL USING (EXISTS (SELECT 1 FROM public.documents d WHERE d.id = document_chunks.document_id AND d.owner_id = auth.uid()));
DROP POLICY IF EXISTS owner_conversations ON public.conversations;
CREATE POLICY owner_conversations ON public.conversations FOR ALL USING (owner_id = auth.uid()) WITH CHECK (owner_id = auth.uid());
DROP POLICY IF EXISTS owner_messages ON public.messages;
CREATE POLICY owner_messages ON public.messages FOR ALL USING (EXISTS (SELECT 1 FROM public.conversations c WHERE c.id = messages.conversation_id AND c.owner_id = auth.uid()));
